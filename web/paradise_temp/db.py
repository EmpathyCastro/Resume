...
if app.config["MONGO_USE_DEV_DB"]:
    CONNECTION_STRING = app.config["MONGO_DEV_DB_URI"]
else:
    CONNECTION_STRING = app.config["MONGO_DATABASE_URI"]
CLIENT = MongoClient(CONNECTION_STRING, tlsCAFile=certifi.where())
SESSION = None

if app.config["MONGO_USE_DEV_DB"]:
    DB = CLIENT.dev_paradise
else:
    DB = CLIENT.paradise
...
class BaseDocument:
    ...
    def create_many(cls, raw_documents, push=True):
        ...
        for doc in raw_documents:
            obj = cls(doc)
            obj.check_validity()
            documents.append(obj._document)

        if push:
            results = cls.collection.insert_many(documents, session=SESSION)
            for i, doc in enumerate(raw_documents):
                doc["_id"] = results.inserted_ids[i]
                doc.update(documents[i])

        def generator(docs):
            for inner_doc in docs:
                yield cls(inner_doc)
        return generator(raw_documents)
    ...
    def is_on_db(self):
        return bool(self._document.get("_id"))
            ...
            if not key_by:
                ...
                for doc in documents:
                    ...
                    if isinstance(key, ObjectId):
                        key = str(key)
                    if not key_single and key not in docs:
                        docs[key] = []
                    if not key_single:
                        docs[key].append(doc)
                    else:
